<div class="col p-0">
    <div class="text-justify h-100 d-flex flex-column justify-content-between">
        <div class="card-body pb-0 pl-0 pt-0">
            <x-adaptive-image height='450px'
                src='{{$item->preview_image ? "/upload/{$item->preview_image}" : "/no-image.jpg"}}' />
            <h4 class="card-title font-weight-bold pt-1">{{$item->title}}</h4>
            <p class="card-subtitle mb-2 text-muted">
                {{date('d.m.Y H:m', strtotime($item->published_at))}} <a href="{{ route('profile.show', ['id'=>$item->user()->get()->first()->id]) }}"> {{$item->user()->get()->first()->name . ' ' . $item->user()->get()->first()->surname}} </a></p>
